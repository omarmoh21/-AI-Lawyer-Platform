...
(images and/or a single PDF) using Gemini 2.5 Flash for OCR, then pauses for
...
at most one PDF, as long as the combined page count (PDF page count + one
page per image) does not exceed MAX_TOTAL_PAGES. Files are treated as pages
of a single logical document and processed in upload order.
...
OCR_PROMPT_TEMPLATE = (
    "Below are {n} item(s) (each either an image or a single-page PDF) that need "
    "their text extracted via OCR.\n"
    "Instructions:\n"
    "- Preserve line breaks.\n"
    "- Preserve punctuation and numbers exactly.\n"
    "- Do NOT correct spelling mistakes.\n"
    "- Do NOT translate the text.\n"
    "- Separate the extracted text of each item with the exact delimiter line:\n"
    f"{PAGE_BREAK_DELIMITER}\n"
    "- Output ONLY the extracted text for the {n} item(s), in order, nothing else."
)
...
def _build_page_units(file_paths: list[str]) -> list[PageUnit]:
    ...
    units (one per image, one per PDF page), checking the combined page cap."""
    if not file_paths:
    ...
    if len(units) > MAX_TOTAL_PAGES:
        raise ValueError(
            f"Total page count is {len(units)}; only up to {MAX_TOTAL_PAGES} page(s) "
            "are supported across all uploaded file(s)."
        )
...
def _run_ocr(units: list[PageUnit], client: genai.Client, feedback: str) -> None:
    """Fill in `text` (in place) for every unit that needs OCR, via one batched Gemini request."""
    ...
    prompt = OCR_PROMPT_TEMPLATE.format(n=len(ocr_indices))
    if feedback:
        prompt += f"\n\nThe previous extraction was rejected with this feedback: {feedback}\nPlease re-extract, taking the feedback into account."
    ...
    chunks = [
        chunk.strip() for chunk in response.text.strip().split(PAGE_BREAK_DELIMITER)
    ]

    if len(chunks) != len(ocr_indices):
        # Gemini didn't follow the delimiter instruction exactly; fall back to
        # treating the whole response as one block for the first OCR'd item.
        chunks = [response.text.strip()] + [""] * (len(ocr_indices) - 1)

    for i, chunk in zip(ocr_indices, chunks):
        units[i]["text"] = chunk
...
def extract_text_node(state: OcrState) -> dict:
    ...
    if ocr_needed_count:
        ...
        _run_ocr(units, client, feedback)
...
def run_ocr_with_human_review(file_paths: list[str]) -> str:
